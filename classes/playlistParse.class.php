...
class PlaylistParse {
    ...
    function parseToPlaylist(stdClass $parseObj) {
        ...
        if ($parseObj->fromUser != null) $playlist->setFromUser($parseObj->fromUser);
	...
	public function savePlaylist(Playlist $playlist) {
		try {
			...
			if ($playlist->getObjectId() == '') {
				...
				if ($playlist->getActive() != null) $parseObject->active = $playlist->getActive();				
				if ($playlist->getFromUser() != null) $parseObject->fromUser = $playlist->getFromUser();
				if ($playlist->getName() != null) $parseObject->name = $playlist->getName();
				if ($playlist->getSongs() != null) $parseObject->songs = $playlist->getSongs();
				if ($playlist->getUnlimited() != null) $parseObject->unlimited = $playlist->getUnlimited();
				if ($playlist->getACL() != null) $parseObject->ACL = $playlist->getACL()->acl;
