...
class SignupController extends REST {
    ...
    public function signup() {
        ...
        try {
            ...
            if ($user == false) {
                $endTimer = microtime();
                jamLog(__FILE__, __LINE__, '[Execution time: ' . executionTime($startTimer, $endTimer) . '] Error during signup "Unable to excute insertUser"');
                $this->response(array('status' => $controllers['NEWUSERCREATIONFAILED']), 503);                
            }
            $node = createNode($connection, 'user', $user->getId());
            if ($node == false) {
                $endTimer = microtime();
                jamLog(__FILE__, __LINE__, '[Execution time: ' . executionTime($startTimer, $endTimer) . '] Error during signup "Unable to excute createNode"');
                $this->response(array('status' => $controllers['NEWUSERCREATIONFAILED']), 503);
            } else {
                $connection->commit();
