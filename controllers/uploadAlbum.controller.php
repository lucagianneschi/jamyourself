...
//utente non loggato
    ...
    public function createAlbum() {
        try {
            ...
            $this->debug("createAlbum", "creating folder for album... ", null);
            if (!$this->createFolderForAlbum($currentUser->getObjectId(), $albumId)) {
                $this->debug("createAlbum", "creating folder for album... ERROR", null);
                rollbackUploadAlbumController($albumId, "Album");
                $this->debug("createAlbum", "RETURN ---------------------------------------------------------------------------------------", null);
                $this->response(array("status" => $controllers['ALBUMNOTSAVED']), 408);
            }
            $this->debug("createAlbum", "creating folder for album... OK", null);
