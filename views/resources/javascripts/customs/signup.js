...
$(document).ready(function() {
    ...
    /*
     * gestione event click per aumentare il numero di componenti della band
     */
    var numComponent = 3;
    $('.addComponents').click(function() {
    	try{
			...
			window.console.error("Event click .addComponents | An error occurred - message : " + err.message);
		}
    });
	...
	try{
		$('#signup01-signup01 .signup-button').click(function() {
	        ...
	        if (!espressione.test($('#signup01-username').val())) {
	        	...
	        	var exp = new RegExp(/(\s)$/);
	        	if(exp.test($('#signup01-username').val())) console.log('SPAZIO ALLA FINE');
	        	
	        	var exp = new RegExp(/^[!""#$%&'()*+,-./:;<=>?[\]^_`{|}~]/);
	        	if(!exp.test($('#signup01-username').val())) console.log('CARETTERI SPECIALI');
