...
//        
		...
		if (self.opt.autohidemode === false) {
		    ...
		    if (self.railh)
			self.autohidedom = self.autohidedom.add(self.railh.cursor);
		}
...
//                      fx = 0;
			...
			self.onmousemove = function(e) {
				...
				self.synched('mousemove', function() {
					...
					self.showCursor();
					if (self.rail.drag.hr)
					    self.doScrollLeft(Math.round(self.scroll.x * self.scrollratio.x));
					else
			...
			if (self.opt.sensitiverail) {
			    self.rail.click(function(e) {
				self.doRailClick(e, false, false)
			    });
			    self.rail.dblclick(function(e) {
				self.doRailClick(e, true, false)
			    });
			    self.cursor.click(function(e) {
				self.cancelEvent(e)
			    });
			    self.cursor.dblclick(function(e) {
				self.cancelEvent(e)
			    });
			}
			...
			if (!self.isiframe)
			    self.bind((cap.isie && self.ispage) ? document : self.docscroll, "mousewheel", self.onmousewheel);
...
//
	...
	this.resize = function() {
	    self.delayed('resize', self.onResize, 30);
	    return self;
	} // event debounce
	...
	function execScrollWheel(e, hr) {
		...
		self.lastdeltax += px;
		self.synched("mousewheelx", function() {
